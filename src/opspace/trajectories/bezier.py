...
from src.opspace.utils.errors import OptimizationError
...
class BezierCurve:
    ...
    def arc_length(self, num_samples: int = 1000) -> float:
        """
        Computes the approximate length of the Bézier curve by sampling points.

        Args:
            num_samples (int): Number of samples to use for numerical approximation.

        Returns:
            float: Approximate length of the Bézier curve.
        """
        t_values = np.linspace(self.a, self.b, num_samples)
        sampled_points = self(t_values)
