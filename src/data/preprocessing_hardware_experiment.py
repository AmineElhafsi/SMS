...
class DataPreprocessor:
    ...
    def _run_video_segmentation(self):
        ...
        ####
        # crop images to get central region
        crop_width = 300 # from either side
        crop_height = 0 # from top and bottom
        color_cropped = color[:, crop_width:-crop_width]
        depth_cropped = depth[:, crop_width:-crop_width]
        # # breakpoint()
        detections, boxes = self._get_detections(0, color_cropped, depth_cropped, T_c2w)
        #####
        ...
        #####
        # get box centers
        boxes[:, 0] = boxes[:, 0] + crop_width
        boxes[:, 2] = boxes[:, 2] + crop_width
        boxes[:, 1] = boxes[:, 1] + crop_height
        boxes[:, 3] = boxes[:, 3] + crop_height

        # get box centers
        centers_x = (boxes[:, 0] + boxes[:, 2]) / 2 + 5
        centers_y = (boxes[:, 1] + boxes[:, 3]) / 2 - 5
        points = np.stack((centers_x, centers_y), axis=1)
        # breakpoint()
        # convert color image to PIL
        from PIL import Image
        color = Image.fromarray(color)
        show_detections(color, detections, boxes, save_path=self.data_directory / "debugging" / "bboxes" / f"{0}_uncropped.png")
        #####
        ...
        ###
        # video_segmentation = self.segmentation_model.segment_video(str(self.data_directory / "rgb_jpg"), points=points)
        ###
    ...
    def _complete_video_frame_segmentation(self, idx, color, depth, T_c2w, K, frame_segmentation):
        # segment ground
        # ground_mask = get_ground_mask(depth, T_c2w, K) # ground height should be taken from config
        ...
        # everything that is not a foreground mask is other
        other_mask = np.zeros_like(depth)
        for i in range(len(foreground_masks)):
            other_mask[foreground_masks[i]] = 1
        other_mask = np.logical_not(np.logical_or.reduce(foreground_masks))
        ...
        masks = [mask.squeeze().astype(bool) for mask in foreground_masks] + [other_mask]
        for i, mask in enumerate(masks):
            segmentation[mask] = int(i)

        assert -1 not in segmentation, "Segmentation contains -1 values, which means some pixels are not assigned to any class."
        ...
        print("Unique values in segmentation: ", np.unique(refined_segmentation))
    ...
    def _get_detections(self, idx, color, depth, T_c2w):
        ...
        if self.preprocessing_config["detection"]["mask_image_background"]:
            detection_query = get_image_foreground(color, depth, distance_threshold=2.5, bg_color=255)
        else:
